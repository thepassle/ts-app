...
class ShoppingApp extends LitElement {
  ...
  pricefilter: boolean = true;
	...
	*/
	_sortAscOrDesc(a: Product, b: Product): number {
		if(this.pricefilter) {
	...
	*/
	_filterSizes(size: string): boolean {
	...
	_handleSizes(e: CustomEvent) {
		this.sizes[e.detail] = !this.sizes[e.detail];
		this.requestUpdate();
	}

	/** Adds product to the cart, if its already in the cart, update the quantity */
	_addToCart(e: CustomEvent) {
		const alreadyInCart = this.shoppingcart.some((product: Product): boolean => product.id === e.detail);
		if (alreadyInCart) {
			this.shoppingcart = this.shoppingcart.map((product: Product): Product => {
			...
			this.shoppingcart = this.products.reduce((acc: Product[], product: Product): Product[] =>  {
				if(product.id === e.detail) {
	...
	_removeFromCart(e: CustomEvent): void {
	...
	render() {
		return html`
			<shopping-product-list
				@orderbychanged=${(e: CustomEvent) => { this.pricefilter = e.detail }}
				...
				.products=${this.products
					...
					.sort((a: Product, b: Product): number => this._sortAscOrDesc(a, b))
        }></shopping-product-list>
			<shopping-cart
